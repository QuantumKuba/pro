 ...
 * - Set take profit (second click - below entry for short, also defines right edge)
 * - Set stop loss (third click - above entry for short)
...
const shortPosition: OverlayTemplate = {
  ...
  createPointFigures: ({ coordinates, overlay, chart }) => {
    ...
    const entryColor = '#FF9800'  // Orange for short entry
    ...
    // Take Profit (second point - below entry for short)
    if (coordinates.length > 1 && points[1]) {
      const tpCoord = coordinates[1]
      const tpPrice = points[1].value || 0
      const profitDiff = entryPrice - tpPrice // Reversed for short
    ...
    // Stop Loss (third point - above entry for short)
    if (coordinates.length > 2 && points[2]) {
      ...
      const lossDiff = slPrice - entryPrice // Reversed for short
      ...
      // Calculate Risk/Reward Ratio (reversed for short)
